package vezbi.ApplicantEvaluation13;
    ...
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        switch (type) {
            ...
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE:
                return new AndEvaluator(new NoCriminalRecordEvaluator(), new MoreExperienceEvaluator());

            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE:
                return new AndEvaluator(new MoreExperienceEvaluator(), new MoreCreditScoreEvaluator());

            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE:
                return new AndEvaluator(new NoCriminalRecordEvaluator(), new MoreCreditScoreEvaluator());
...
class AndEvaluator implements Evaluator {
    private Evaluator first;
    private Evaluator second;

    public AndEvaluator(Evaluator first, Evaluator second) {
        this.first = first;
        this.second = second;
    }
    ...
    @Override
    public boolean evaluate(Applicant applicant) {
        return first.evaluate(applicant) && second.evaluate(applicant);
    }
}

class InvalidEvaluation extends Exception {
    public InvalidEvaluation() {
        super("Invalid Evaluation Type");
    }
}
