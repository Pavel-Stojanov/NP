...
//TODO constructor
    ...
    LogProcessor<E> firstLogProcessor = l->l.stream()
            .filter(log->log.getType().equals("INFO"))
            .collect(Collectors.toCollection(ArrayList::new));

    LogProcessor<E> secondLogProcessor = log->log.stream()
            .filter(l->l.getMessage().length()<100)
            .collect(Collectors.toCollection(ArrayList::new));

    LogProcessor<E> thirdLogProcessor = log->log.stream()
            .sorted()
            .collect(Collectors.toCollection(ArrayList::new));
