...
class University {
    ...
    public Optional<Department> getMostPopularDepartment() {
        ...
        return departments.stream()
                .max(Comparator.comparing(department -> department.getCourses()
                        .stream()
                        .mapToInt(Course::getEnrolledStudents)
                        .sum()));
    }
    ...
    public List<Course> getCoursesByDifficultyRange(int min, int max) {
        ...
        return departments.stream()
                .flatMap(department -> department.getCourses().stream())
                .filter(course -> course.getDifficulty() <= max && course.getDifficulty() >= min)
                .collect(Collectors.toList());
    }
    ...
    public University mergeFourSmallestDepartments() {
        ...
        List<Department> sorted = departments.stream().sorted(Comparator.comparing(department -> department.getCourses().stream()
                        .mapToInt(Course::getEnrolledStudents).sum()))
                .collect(Collectors.toList());


        List<Department> remaining = sorted.stream().skip(4).collect(Collectors.toList());

        sorted.stream().limit(4).reduce(((department1, department2) -> new Department(
                department1.getName() + " & " + department2.getName(),
                Stream.concat(department1.getCourses().stream(), department2.getCourses().stream()).collect(Collectors.toList())
